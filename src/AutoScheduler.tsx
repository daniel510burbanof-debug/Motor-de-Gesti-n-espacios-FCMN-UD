...
function runScheduler(
  ...
  // 1. PRIORIZACIÓN EQUILIBRADA: Las teorías largas compiten con los laboratorios
  const difficultyScore = (r: ClassRequest): number => {
    let score = 0;
    // Priorizamos bloques horarios grandes (más difíciles de encajar)
    score += r.hoursBlock * 250; 
    // Los laboratorios tienen un plus para asegurar equipo, pero no de 1000
    if (r.type === "Laboratorio") score += 300; 
    if (r.diasDisponibles && r.diasDisponibles.length > 0)
      score += (6 - r.diasDisponibles.length) * 100;
    if (r.espacioEspecifico) score += 500;
    score += r.students;
    return score;
  };
  ...
  // 2. BUSQUEDA DE CANDIDATOS (CON SOPORTE PARA CASCADA)
  const findCandidates = (
    req: ClassRequest,
    pool: RoomEntry[],
    respectarGap: boolean,
    respectarSede: boolean, // <-- Se vuelve dinámico
    ...
    let diasBase = (req.diasDisponibles && req.diasDisponibles.length > 0)
    ...
    for (const day of diasBase) {
      // RESTRICCIÓN DE SEDE/COHORTE: Ahora se puede ignorar en la cascada
      if (respectarSede) {
      ...
      for (let hi = 0; hi <= HOURS.length - req.hoursBlock; hi++) {
        ...
        if (req.parentId) {
          if (parentSlots.some(ps => ps.day === day && ps.hours.some(h => block.includes(h)))) continue;
        }

        if (req.type === "Laboratorio" && teoriaOcup.some(ts => ts.day === day && ts.hours.some(h => block.includes(h)))) continue;
        ...
        if (respectarGap) {
          ...
          if (existentes.length > 0 && maxGap < 999) {
            const startIdx = getHourIndex(start);
            const minExist = Math.min(...existentes);
            const maxExist = Math.max(...existentes);
            if ((startIdx - maxExist - 1) > maxGap || (minExist - (startIdx + req.hoursBlock)) > maxGap) continue;
          }
        }
        ...
        for (const room of sortedPool) {
          ...
          if (externalSpaces) {
            ...
            if (ext) {
              const ap = String(ext.hora_apertura || "06:00").substring(0, 5);
              const ci = String(ext.hora_cierre || "19:00").substring(0, 5);
              if (block.some(h => h < ap || h >= ci)) continue;
            }
          }
          ...
          if (req.type === "Laboratorio" && labAvailability.length > 0) {
            ...
            if (ventanas.length > 0 && !ventanas.some(v => block.every(h => h >= v.from && h < v.to))) continue;
          }
  ...
  const confirmarAsignacion = (req: ClassRequest, best: any) => {
    const cohortKey = `${req.program}__${req.cohort}`;
    const { day, hour, block, room } = best;
    block.forEach((h: string) => {
    ...
    block.forEach((h: string) => cohortDayHours[cohortKey][day].push(getHourIndex(h)));
    ...
    if (req.type === "Teoría") {
      const tk = `${cohortKey}__${req.subject}`;
      if (!teoriaSlots[tk]) teoriaSlots[tk] = [];
      teoriaSlots[tk].push({ day, hours: block });
    }
    if (req.parentId) {
      ...
      parentRoomPreference[req.parentId] = room.name;
    }
    if (req.hoursBlock >= 4) {
      const bi = getHourIndex(hour) + req.hoursBlock;
      if (bi < HOURS.length) teacherBreak[day][HOURS[bi]][req.teacher] = true;
    }
    roomUsageCount[room.name] = (roomUsageCount[room.name] || 0) + 1;
    assignments.push({
      request: req, day, hour, hour_end: block[block.length-1],
  ...
  // 3. LOOP PRINCIPAL CON CASCADA DE RESCATE
  for (const req of sorted) {
    let basePool = req.type === "Laboratorio" ? labPool : teoriaPool.filter(r => r.subtipo === req.tipoEspacio);
    if (basePool.length === 0 && req.type === "Teoría") basePool = teoriaPool;

    let pool = basePool.filter(r => r.capacity >= req.students && (!externalSpaces || externalSpaces.find((s:any)=>s.nombre===r.name)?.activo !== false));
    if (req.espacioEspecifico) {
      const forced = [...teoriaPool, ...labPool].find(r => r.name === req.espacioEspecifico);
      if (forced) pool = [forced];
    }

    // Cascada: Intentamos lo más restrictivo primero, luego relajamos reglas
    let candidates = findCandidates(req, pool, true, true, false); // 1. Ideal
    if (!candidates.length) candidates = findCandidates(req, pool, false, true, false); // 2. Sin Gap
    if (!candidates.length) candidates = findCandidates(req, pool, true, false, false); // 3. Permitir Teoría+Lab mismo día
    if (!candidates.length) candidates = findCandidates(req, pool, false, false, false); // 4. Sin nada (semana)
    if (!candidates.length) candidates = findCandidates(req, pool, false, false, true); // 5. Sábado desesperado

    if (candidates.length > 0) {
      candidates.sort((a, b) => b.score - a.score);
      confirmarAsignacion(req, candidates[0]);
    } else {
      conflicts.push({ request: req, reason: "Sin espacio: Docente ocupado o capacidad insuficiente.", type: "hard" });
    }
  }
...
const PROG_COLORS: Record<string,string> = { "Química":"#F472B6","Biología":"#4ADE80","Física":"#60A5FA","Matemáticas":"#FB923C" };
const PROG_ICONS: Record<string,string> = { "Química":"⚛️","Biología":"🧬","Física":"🧲","Matemáticas":"π" };
const DEFAULT_PROGRAM_CONFIG: ProgramConfig[] = [ {program:"Química",maxGapHours:3},{program:"Biología",maxGapHours:3}, {program:"Física",maxGapHours:3},{program:"Matemáticas",maxGapHours:3} ];
...
export default function AutoScheduler({session,onClose,onSaved,spaces:externalSpaces}:AutoSchedulerProps) {
  ...
  const Sty = {
    ...
    box:{background:T.bg3,borderRadius:16,borderWidth:1,borderStyle:"solid",borderColor:T.border2,width:"100%",maxWidth:980,maxHeight:"93vh",overflowY:"auto" as const,boxShadow:T.shadow},
    inp:{background:T.inputBg,borderWidth:1,borderStyle:"solid",borderColor:T.inputBorder,color:T.text,borderRadius:8,padding:"7px 10px",fontSize:16,outline:"none",width:60,textAlign:"center" as const,minHeight:44},
      ...
      <div style={Sty.box}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 24px",borderBottom:`1px solid ${T.border}`}}>
          <div>
            ...
            <div style={{fontSize:11,color:T.muted,marginTop:3}}>Asignación Inteligente con Cascada de Rescate</div>
          </div>
        ...
        <div style={{display:"flex",borderBottom:`1px solid ${T.border}`}}>
          {[{key:"upload",label:"1. Subir Excel"},{key:"config",label:"2. Configurar"},{key:"preview",label:"3. Vista Previa"},{key:"done",label:"4. Guardado"}].map(s=>(
            <div key={s.key} style={{flex:1,padding:"10px",textAlign:"center" as const,fontSize:12,fontWeight:600, color:step===s.key?"#60a5fa":T.muted, borderBottom:step===s.key?`2px solid ${T.udAccent}`:"2px solid transparent", background:step===s.key?"rgba(0,102,204,0.08)":"transparent"}}>
              {s.label}
            </div>
          ))}
        </div>
        ...
        <div style={{padding:24}}>
          {step==="upload"&&(
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              <div onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop} onClick={()=>document.getElementById("xl-input-v2")?.click()}
                style={{borderWidth:2,borderStyle:"dashed",borderColor:dragOver?T.udAccent:T.border2,borderRadius:12,padding:48,textAlign:"center" as const,background:dragOver?"rgba(0,102,204,0.08)":T.bg2,cursor:"pointer"}}>
                <div style={{fontSize:48,marginBottom:12}}>📊</div>
                <div style={{fontSize:15,fontWeight:700,color:T.text}}>Arrastra tu Excel aquí</div>
                <input id="xl-input-v2" type="file" accept=".xlsx,.xls" style={{display:"none"}} onChange={handleFile}/>
              </div>
              {error&&<div style={{background:"rgba(239,68,68,0.1)",color:"#f87171",padding:12,borderRadius:8}}>⚠ {error}</div>}
            </div>
          )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,1fr)",gap:12}}>
                {programConfig.map((pc,i)=>(
                  <div key={pc.program} style={{background:T.bg,borderRadius:8,padding:16,display:"flex",alignItems:"center",justifyContent:"space-between",border:`1px solid ${T.border}`}}>
                    <span style={{fontWeight:600,color:PROG_COLORS[pc.program]}}>{PROG_ICONS[pc.program]} {pc.program}</span>
                    <input type="number" value={pc.maxGapHours} onChange={e=>setProgramConfig(prev=>prev.map((p,j)=>j===i?{...p,maxGapHours:parseInt(e.target.value)||0}:p))} style={Sty.inp}/>
                  </div>
                ))}
              </div>
              <button onClick={generateSchedule} style={{...Sty.btn(`linear-gradient(135deg,${T.udBlue},${T.udAccent})`),width:"100%"}}>🚀 Generar Horario</button>
            </div>
          )}
          ...
          {step==="preview"&&(
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>setFilterView("all")} style={Sty.btn(filterView==="all"?T.udBlue:T.bg2)}>Todas</button>
                <button onClick={()=>setFilterView("teoria")} style={Sty.btn(filterView==="teoria"?T.udBlue:T.bg2)}>🏫 Teoría</button>
                <button onClick={()=>setFilterView("lab")} style={Sty.btn(filterView==="lab"?T.udBlue:T.bg2)}>🔬 Labs</button>
              </div>

              <div style={{maxHeight:400,overflowY:"auto" as const,border:`1px solid ${T.border}`,borderRadius:8}}>
                <table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}>
                  <thead style={{background:T.bg3,position:"sticky",top:0}}>
                    <tr>
                      <th style={{padding:10,textAlign:"left"}}>Tipo</th>
                      <th style={{padding:10,textAlign:"left"}}>Asignatura</th>
                      <th style={{padding:10,textAlign:"left"}}>Día/Horario</th>
                      <th style={{padding:10,textAlign:"left"}}>Espacio</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((a,i)=>(
                      <tr key={i} style={{borderBottom:`1px solid ${T.border}30`}}>
                        <td style={{padding:10}}>{a.tipo_espacio==="lab"?"🔬":"🏫"}</td>
                        <td style={{padding:10}}>{a.displayLabel}</td>
                        <td style={{padding:10}}>{a.day} ({a.hour})</td>
                        <td style={{padding:10,fontWeight:600}}>{a.room}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {hardConflicts.length>0&&(
                <div style={{padding:16,background:"rgba(239,68,68,0.1)",borderRadius:8}}>
                  <div style={{fontWeight:700,color:"#f87171"}}>Conflictos detectados: {hardConflicts.length}</div>
                  {hardConflicts.slice(0,5).map((c,i)=><div key={i} style={{fontSize:11,color:T.mutedL}}>• {c.request.subject}: {c.reason}</div>)}
                </div>
              )}

              <button onClick={handleSave} disabled={saving} style={{...Sty.btn(`linear-gradient(135deg,${T.udBlue},${T.udAccent})`),width:"100%"}}>
                {saving?"Guardando...":`✅ Confirmar ${assignments.length} reservas`}
              </button>
            </div>
          )}
          ...
          {step==="done"&&(
            <div style={{textAlign:"center",padding:40}}>
              <h2>¡Listo! 🎉</h2>
              <p>{savedCount} asignaciones guardadas correctamente.</p>
              <button onClick={onClose} style={Sty.btn(T.udBlue)}>Volver al tablero</button>
            </div>
          )}
        </div>
      </div>
